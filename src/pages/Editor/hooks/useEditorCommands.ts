  ...
  const editor = useMemo(() => {
    const focusChain = {
      ...
      insertBlock: (type: string) => {
        ...
        setTimeout(() => {
          if (blocksRefs?.current?.[newId]) {
            blocksRefs.current[newId].focus();
            blocksRefs.current[newId].scrollIntoView({ behavior: 'smooth', block: 'center' });
