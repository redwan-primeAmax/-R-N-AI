...
// Allocating static thread-safe buffer up to maximum word limit to guarantee absolutely zero garbage collection (GC) sweeps during searches.
...
/**
 * Super-optimized, zero-allocation Wagner-Fischer Edit Distance inside safe boundaries
 */
export function getFastEditDistance(s1: string, s2: string, maxLimit: number): number {
  ...
  for (let i = 1; i <= len1; i++) {
    ...
    for (let j = 1; j <= n; j++) {
      const char2 = s2[j - 1];
      let val = row[j - 1]; // diagonal substitute cost
