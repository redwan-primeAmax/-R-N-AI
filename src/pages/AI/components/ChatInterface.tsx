      ...
      <header className="px-6 pt-safe-area-inset-top pb-4 flex justify-between items-center bg-[#0d0d0d] sticky top-0 z-30 border-b border-white/5">
        <div className="flex items-center gap-4">
          <button 
            ...
            className="w-11 h-11 flex items-center justify-center bg-[#1a1a1a] rounded-full transition-all active:scale-95 text-white/80 hover:text-white shadow-sm cursor-pointer hover:bg-white/5 relative z-50"
            aria-label="Go back"
          >
            <ChevronLeft size={24} />
          </button>
                  ...
                  {aiStatus === 'generating' ? (
                    ...
                    <div className="flex flex-col gap-2">
                       <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-red-400 uppercase tracking-widest">{aiReason || 'ত্রুটি ঘটেছে'}</span>
                      </div>
                      {(aiReason?.toLowerCase().includes('limit') || aiReason?.toLowerCase().includes('quota') || aiReason?.toLowerCase().includes('socket')) && (
                        <button 
                          onClick={() => navigate('/ai/settings')}
                          className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[9px] font-bold uppercase tracking-wider"
                        >
                          Switch to Local AI (Unlimited)
                        </button>
                      )}
                    </div>
                  ) : (
